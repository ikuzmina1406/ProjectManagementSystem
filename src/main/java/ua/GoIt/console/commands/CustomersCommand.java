...
public class CustomersCommand implements Command {
    ...
    public void printActiveMenu() {
        LOGGER.info("--------Category menu----------" + "\n" + "--------Command menu----------" + "\n" + "create[!name!state_code!country!birthday!sex!info]----read (получить инфо о всех клиентах)----update[!id!name!state_code!country!birthday!sex!info]----delete[!id]-(удалить клиента по id )----get[!id]-(получить инфо о клиентах по id)----");
    }

    private void read() { //to get the list of all customers (the command : "read")
        List<Customers> all = customersDao.read();
    ...
    private void delete(String subParams) { //delete from customers (the command: "delete!24")
        String[] paramsArray = subParams.split("!");
        Optional<Customers> customers = customersDao.get(Long.parseLong(paramsArray[0]));
        if (customers.isPresent()) {
